...
public class SimScenario implements Serializable {
	...
	/** percentage of malicious nodes, which drops messages*/
	private static final String PERCENTAGE_OF_DROPPING_NODES = "percentageOfDroppingNodes";
	...
	/** Global cluster event listeners */
	...
	private final double percentageOfDroppingNodes;
	...
	protected SimScenario() {
		...
		percentageOfDroppingNodes = s.getInt(PERCENTAGE_OF_DROPPING_NODES);
	...
	/**
	 * Adds a new cluster listener for all nodes
	 * @param cl The listener
	 */
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			int appCount;
			int numberOfLeftMaliciousNodesInGroup = (int) ((double)(nrofHosts * (double)(this.percentageOfDroppingNodes / 100)) + 0.5);
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, this.clusterListeners,
						this.movementListeners,	gid, interfaces, comBus,
						mmProto, mRouterProto);
				hosts.add(host);

				if (numberOfLeftMaliciousNodesInGroup > 0) {
					host.setMalicious(true);
					numberOfLeftMaliciousNodesInGroup--;
				}
			}
		}
	}

	private boolean isHostMalicious(int absoluteNumberOfDroppingNodes, int hostCounter) {
		if (absoluteNumberOfDroppingNodes == 0 ) {
			return false;
		}
		return hostCounter % absoluteNumberOfDroppingNodes == 0;
	}
